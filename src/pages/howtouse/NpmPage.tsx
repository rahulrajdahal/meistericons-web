...
export default function NpmPage() {
  ...
  const steps = [
    ...
    },
    {
      ...
      step: (
        <div className="flex items-start justify-between px-4 py-2">
          <code className="text-grey-800 text-lg leading-8 -tracking-[0.0225rem] font-normal">{`mni mni-deleteB`}</code>
          <Tooltip.Provider>
            <Tooltip.Root>
              <Tooltip.Trigger asChild>
                <button title={codeText} onClick={copyCode}>
  ...
  return (
    <HowToUsePage
      image={npm}
      title="MeisterIcons is available on npm as a javaScript library. The library lets you quickly search for the icons you’re looking
for."
      steps={steps}
    />
  );
